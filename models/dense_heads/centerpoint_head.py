...
from core import circle_nms, draw_heatmap_gaussian, gaussian_radius, xywhr2xyxyr
...
class CenterHead(BaseModule):
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, img_metas,**kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            # import cv2
            # import numpy as np
            # preds_map = preds_dict[0]["heatmap"].detach().cpu().numpy()[0][0]
            # heatmap = heatmaps[task_id].detach().cpu().numpy()[0][0]

            # preds_map[len(preds_map) // 2, len(preds_map) // 2] = 255
