...
@HEADS.register_module()
class GaussianAnchor3DHead(BaseModule, AnchorTrainMixin):
    """Anchor head for SECOND/PointPillars/MVXNet/PartA2.
    ...
    def __init__(self,
                 ...
                 loss_bbox=dict(
                     type='SmoothL1Loss', beta=1.0 / 9.0, loss_weight=2.0),
                 loss_dir=dict(type='CrossEntropyLoss', loss_weight=0.2),
                 loss_uncertainty=dict(type='GaussianLoss', loss_weight=0.2),
                 init_cfg=None):
        super().__init__(init_cfg=init_cfg)
        self.in_channels = in_channels
        self.num_classes = num_classes
        ...
        self.loss_bbox = build_loss(loss_bbox)
        self.loss_dir = build_loss(loss_dir)
        self.loss_likelihood = build_loss(loss_uncertainty)
        self.fp16_enabled = False
    ...
    def forward_single(self, x):
        ...
        cls_score = self.conv_cls(x)
        bbox_pred = self.conv_reg(x)
        logstd_pred = self.conv_std(x)
        logstd_pred = torch.clamp(logstd_pred, min=-5, max=3)
        dir_cls_preds = None
        if self.use_direction_classifier:
            dir_cls_preds = self.conv_dir_cls(x)
        return cls_score, bbox_pred, dir_cls_preds, logstd_pred
    ...
    def loss_single(self, cls_score, bbox_pred, dir_cls_preds, logstd_preds,labels,
                    label_weights, bbox_targets, bbox_weights, dir_targets,
                    dir_weights, num_total_samples):
        """Calculate loss of Single-level results.
        ...
        bbox_pred = bbox_pred.permute(0, 2, 3,
                                      1).reshape(-1, self.box_code_size)
        logstd_preds = logstd_preds.permute(0, 2, 3,
                                        1).reshape(-1, self.box_code_size)
        bbox_targets = bbox_targets.reshape(-1, self.box_code_size)
        bbox_weights = bbox_weights.reshape(-1, self.box_code_size)
        ...
        pos_bbox_pred = bbox_pred[pos_inds]
        pos_logstd_pred = logstd_preds[pos_inds]
        pos_bbox_targets = bbox_targets[pos_inds]
        pos_bbox_weights = bbox_weights[pos_inds]
        ...
        if num_pos > 0:
            ...
            loss_loglikelihood = self.loss_likelihood(
                pos_bbox_pred,
                pos_bbox_targets,
                pos_logstd_pred,
                pos_bbox_weights,
            ...
            # No object, then I don't know what is certain offset     
            loss_loglikelihood = (1 / pos_logstd_pred).sum()
        return loss_cls, loss_bbox, loss_dir, loss_loglikelihood
    ...
    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'dir_cls_preds', 'logstd_preds'))
    def loss(self,
             cls_scores,
             bbox_preds,
             dir_cls_preds,
        ...
        # num_total_samples = None
        losses_cls, losses_bbox, losses_dir, loss_likelihood = multi_apply(
            self.loss_single,
            cls_scores,
            bbox_preds,
            dir_cls_preds,
        ...
        return dict(
            ...
            loss_dir=losses_dir, loss_likelihood=loss_likelihood)
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(input_metas)):
            ...
            logstd_preds_list = [
                logstd_preds[i][img_id].detach() for i in range(num_levels)
            ]

            input_meta = input_metas[img_id]
            proposals = self.get_bboxes_single(cls_score_list, bbox_pred_list,
                                               dir_cls_pred_list, logstd_preds_list,
                                               mlvl_anchors,
                                               input_meta, cfg, rescale)
            result_list.append(proposals)
        return result_list
    ...
    def get_bboxes_single(self,
        ...
        mlvl_scores = []
        mlvl_dir_scores = []
        for cls_score, bbox_pred, dir_cls_pred, logstd_pred, anchors in zip(
                cls_scores, bbox_preds, dir_cls_preds, logstd_preds, mlvl_anchors):
            assert cls_score.size()[-2:] == bbox_pred.size()[-2:]
            assert cls_score.size()[-2:] == dir_cls_pred.size()[-2:]
            dir_cls_pred = dir_cls_pred.permute(1, 2, 0).reshape(-1, 2)
            dir_cls_score = torch.max(dir_cls_pred, dim=-1)[1]
            ...
            bbox_pred = bbox_pred.permute(1, 2,
                                          0).reshape(-1, self.box_code_size)
            std = logstd_pred.permute(1, 2,
                                        0).reshape(-1, self.box_code_size).exp()
            ...
            bboxes = self.bbox_coder.decode(anchors, bbox_pred)
            mlvl_bboxes.append(torch.cat([bboxes, std], dim = 1))
            mlvl_scores.append(scores)
            mlvl_dir_scores.append(dir_cls_score)

        mlvl_bboxes = torch.cat(mlvl_bboxes)
        mlvl_bboxes_for_nms = xywhr2xyxyr(input_meta['box_type_3d'](
            mlvl_bboxes, box_dim=self.box_code_size * 2).bev)
        mlvl_scores = torch.cat(mlvl_scores)
        mlvl_dir_scores = torch.cat(mlvl_dir_scores)
        ...
        bboxes = input_meta['box_type_3d'](bboxes, box_dim=self.box_code_size * 2)
        return bboxes, scores, labels
    ...
    def get_bboxes_onnx_export(self,
        ...
        for img_id in range(1):
            ...
            proposals = self.get_bboxes_single_onnx_export(cls_score_list, bbox_pred_list,
                                               dir_cls_pred_list, logstd_preds_list, mlvl_anchors)
            result_list.append(proposals)
        return result_list
