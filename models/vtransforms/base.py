...
from core.ops.bev_pool import bev_pool
...
class BaseTransform(nn.Module):
    ...
    def bev_pool(self, geom_feats, x):
        ...
        # import cv2
        # import numpy as np
        # kept_img = kept.view(B, N, D, H, W)[0]
        # kept_img = (torch.sum(kept_img, dim=1).detach().cpu().numpy() > 0).astype(np.uint8) * 255
        # cv2.imwrite('left_kept_img.png', kept_img[0])
        # cv2.imwrite('right_kept_img.png', kept_img[1])
        ...
        # ones_image = ones[0,0].detach().cpu().numpy()
        
        # cv2.imwrite('ones_image.png', ones_image)

        # # collapse Z
        final = torch.cat(x.unbind(dim=2), 1)

        # final_image = torch.sum(final[0], dim=0).detach().cpu().numpy()
        # max_value = np.max(final_image)
        # min_value = np.min(final_image)
        # final_image = (final_image - min_value) / (max_value - min_value) * 255
...
class FisheyeTransform(BaseTransform):
    def __init__(
        ...
        super().__init__(in_channels, out_channels, final_size, feature_size, 
                                         xbound, ybound, zbound, dbound, use_points, depth_input,
                                         height_expand, add_depth_features)
        # self.image_size = image_size
    ...
    def get_geometry(
        ...
        **kwargs,
        ...
        B, N = batch_size, len(camera_models)
        D, H, W = self.frustum.shape[0], self.frustum.shape[1], self.frustum.shape[2]

        frum_points = []
        frustum_points = self.frustum.view(D*H*W, 3).transpose(0, 1)
        for direction in camera_seq:
            camera_model = camera_models[direction]

            # backward projection
            points = camera_model.image2cam(frustum_points[:2], frustum_points[2])
            points = camera_model.cam2world(points)
        
            # forward projection
            new_points = camera_model.world2cam(points)
            new_points = camera_model.cam2image(new_points, False)
            points = points.transpose(0, 1)
            keep = (torch.abs(new_points[0] - frustum_points[0]) < 10) * (torch.abs(new_points[1] - frustum_points[1]) < 10)
            
            points[~keep] = torch.FloatTensor([self.xbound[0]-1, self.ybound[0]-1, self.zbound[0]-1]).to(points.device)

            points = points.view(D, H, W, 3)
            frum_points.append(points)
    ...
    def forward(
        ...
        **kwargs,
        ...
        for b in range(batch_size):
            ...
            # inverse aug        # # import pdb; pdb.set_trace()
            cur_coords -= cur_lidar_aug_matrix[:3, 3]
            cur_coords = torch.inverse(cur_lidar_aug_matrix[:3, :3]).matmul(
                cur_coords.transpose(1, 0)
            )
            # lidar2image
            cur_coords = cur        # # import pdb; pdb.set_trace()_lidar2image[:, :3, :3].matmul(cur_coords)
