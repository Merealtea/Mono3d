...
from .imvoxel_neck import ResModule, ResModuleConv
...
@NECKS.register_module()
class DfMNeckMeanPool(nn.Module):
    """Dual-path neck for monocular and stereo bev fusion in DfM.
    ...
    def forward(self, x):
        ...
        assert x.shape[1] == self.in_channels[0] * self.num_frames
        mono_bev_feat = self.mono_layers(x[:, :self.in_channels[0]])
        mono_bev_feat = mono_bev_feat.mean(dim=4, keepdim=True)
        assert mono_bev_feat.shape[-1] == 1
        mono_bev_feat = mono_bev_feat[..., 0].transpose(-1, -2)
        return [mono_bev_feat]
...
class DfMNeckConv(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_frames = num_frames
        self.mono_layers = nn.Sequential(
            ResModuleConv(in_channels[0], norm_cfg=norm_cfg),
            ConvModule(
                in_channels=in_channels[0],
                out_channels=in_channels[1],
                kernel_size=3,
                ...
                norm_cfg=norm_cfg,
                act_cfg=dict(type='ReLU', inplace=True)),
            ResModuleConv(in_channels[1], norm_cfg=norm_cfg),
            ConvModule(
                in_channels=in_channels[1],
                out_channels=in_channels[2],
                kernel_size=3,
                ...
                norm_cfg=norm_cfg,
                act_cfg=dict(type='ReLU', inplace=True)),
            ResModuleConv(in_channels[2], norm_cfg=norm_cfg),
            ConvModule(
                in_channels=in_channels[2],
                out_channels=out_channels,
                kernel_size=3,
