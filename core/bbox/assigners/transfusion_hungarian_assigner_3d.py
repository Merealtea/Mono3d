...
from ..match_costs.builder import MATCH_COST
from ..iou_calculators.builder import IOU_CALCULATORS
import torch
...
@BBOX_ASSIGNERS.register_module()
class TransFusionHungarianAssigner3D(BaseAssigner):
    ...
    def __init__(self,
        ...
        self.reg_cost = MATCH_COST.build(reg_cost)
        self.iou_cost = MATCH_COST.build(iou_cost)
        self.iou_calculator = IOU_CALCULATORS.build(iou_calculator)
    ...
    def assign(self, bboxes, gt_bboxes, gt_labels, cls_pred, train_cfg):
        ...
        if num_gts == 0 or num_bboxes == 0:
            ...
            if num_gts == 0:
                ...
                assigned_gt_inds[:] = 0
            return AssignResult(
                num_gts, assigned_gt_inds, torch.zeros_like(assigned_gt_inds).to(assigned_gt_inds.device), labels=assigned_labels)
