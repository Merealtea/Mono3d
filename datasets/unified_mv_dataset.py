...
class UnifiedMV3DDataset(BaseDataset):
    """Waymo Dataset.
    ...
    def __init__(self,
        ...
        datasets = os.listdir(data_info_pickles)
        self.data_infos = []
        for dataset in datasets:
            if test_mode:
                data_info = os.path.join(data_info_pickles, dataset,
                                         f"{dataset}_infos_val.pkl")
            else:
                data_info = os.path.join(data_info_pickles, dataset,
                                         f"{dataset}_infos_train.pkl")
            # with open(data_info, 'rb') as f:
            data_info = mmcv.load(data_info)
            self.data_infos.extend(data_info)
        
        self.fisheye_cam_models =\
            dict(zip(['Rock', 'Hycan'],
                     [dict(zip(['left', 'right', 'front', 'back'], 
                        [CamModel(dir, vehicle, 'torch', 'cuda') for dir in ['left', 'right', 'front', 'back']]))
                            for vehicle in ['Rock', 'Hycan']]
                     )) 
        ...
        # 这里需要两个pipeline，分别i是处理图片，处理深度图的 TBD
        self.pipeline = Compose(pipeline)

    def get_single_item(self, idx):

        # idx = idx % 64 # overfiting test

        single_data_info = self.data_infos[idx]
        cam_info = single_data_info['key_frame_cam_info']
        direction = list(cam_info.keys())
        
        # Process the original image and ground truth
        img_metas = {
            "dataset_type": single_data_info['dataset_type'],
            "direction": direction,
            "num_ref_num" : self.num_ref_frames,
            "extrinsic": dict(zip(direction, [cam_info[dir]['extrinsic'] for dir in direction])),
            "intrinsic": dict(zip(direction, [cam_info[dir]['intrinsic'] for dir in direction])),
            "key_frame_ego_pose" : single_data_info['key_frame_ego_pose'],
            "timestamp" : single_data_info['timestamp'],
        }
        
        valid_class_mask = [cat in self.CLASSES for cat in single_data_info['gt_labels']]
        gt_labels = [self.CLASSES.index(cat) if cat in self.CLASSES else 0 for cat in single_data_info['gt_labels']]

        ann_info = {
            "gt_bboxes_3d": np.array(single_data_info['gt_boxes'])[valid_class_mask][:, :7],
            "gt_labels_3d": np.array(gt_labels)[valid_class_mask],
            "gt_labels": np.array(gt_labels)[valid_class_mask],
        }

        img_info = {
            "img_filename": [cam_info[dir]['image_path'] for dir in direction],
        }

        # depth_info = {
        #     "depth_filename": [cam_info[dir]['depth_path'] for dir in direction],
        #     "depth_img" : []
        # }

        processed_key_frame_image = self.pipeline(dict(
                img_info=img_info,
                # depth_info=depth_info,
            ))

        # ratio = processed_key_frame_image['scale']
        # pad_shape = processed_key_frame_image['pad_shape'][0]
        
        # for depth_path in depth_info['depth_filename']:
        #     depth = np.load(depth_path)
        #     shape = depth.shape
        #     new_shape = (int(shape[0] * ratio), int(shape[1] * ratio))
        #     # downsample
        #     downsampled_depth = cv2.resize(depth, (new_shape[1], new_shape[0]), interpolation=cv2.INTER_NEAREST)

        #     # pad
        #     x_pad, y_pad = pad_shape[0] - new_shape[0], pad_shape[1] - new_shape[1]
        #     downsampled_depth = np.pad(downsampled_depth, ((0, x_pad), (0, y_pad)), mode='constant', constant_values=0)

        #     # quantization
        #     # min_depth, max_depth, depth_interval = self.dbound
        #     # invalid_part = downsampled_depth == 0
        #     # downsampled_depth[downsampled_depth < min_depth] = min_depth
        #     # downsampled_depth[downsampled_depth > max_depth] = max_depth
        #     # downsampled_depth = downsampled_depth // depth_interval
        #     # downsampled_depth[invalid_part] = 0

        #     depth_info['depth_img'].append(downsampled_depth)
        
        # Pad image to same length
        processed_key_frame_image['img'] = processed_key_frame_image['img'] 
        processed_key_frame_image['img'] = np.stack(processed_key_frame_image['img'], axis=0)


        if self.num_ref_frames > 0:
            img_metas["cam_sweeps_ego_pose"] = [cam_sweep_info['ego_pose'] for cam_sweep_info in single_data_info['cam_sweeps']]
            img_metas["cam_sweeps_timestamp"] = [cam_sweep_info['timestamp'] for cam_sweep_info in single_data_info['cam_sweeps']]

            cam_sweeps_info = [
                [cam_sweep_info['cam_info'][dir]['image_path'] for dir in direction] for cam_sweep_info in single_data_info['cam_sweeps']
            ]


            processed_sweep_image = [
                self.pipeline(dict(
                    img_info= {"img_filename" : sweeps_info}
                    # depth_info={}
                )) for sweeps_info in cam_sweeps_info
            ]

            processed_sweep_image = [np.stack(processed_sweep_image[i]['img'] + [np.zeros_like(processed_sweep_image['img'][0])] * (6-len(processed_sweep_image['img'])), axis=0) for i in range(len(processed_sweep_image))]
            processed_key_frame_image['img'] = np.concatenate([processed_key_frame_image['img']] + processed_sweep_image, axis=0)
            
        results = dict()
        results.update(processed_key_frame_image)
        # results.update(depth_info)
    ...
    def collate(self, results):
        ...
        for result in results:
            imgs.append(result['img'].transpose(0, 3, 1, 2))
            ...
            # depth_img.append(result['depth_img'])
            timestamps.append(result['img_metas']['timestamp'])
        ...
        # batch_results['depth_img'] = torch.LongTensor(np.stack(depth_img))
        batch_results['timestamp'] = timestamps 
        
        batch_results["img_metas"] = []
        for res in results:
            img_meta = res['img_metas']
            for key in res:
